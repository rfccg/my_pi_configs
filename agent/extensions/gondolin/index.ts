...
export default function (pi: ExtensionAPI) {
	...
	async function startVm(ctx?: ExtensionContext): Promise<VM> {
		...
		const piIgnorePolicy = createPiIgnorePolicySync(localCwd);
		const filteredWorkspace = createFilteredFSProvider(new RealFSProvider(localCwd), localCwd, piIgnorePolicy, {
		...
		ctx?.ui.notify(
			`Gondolin VM ready. ${localCwd} is mounted at ${GUEST_WORKSPACE}; .pi-ignore is enforced in the VM.`,
			"info",
		);
		return created;
	}
